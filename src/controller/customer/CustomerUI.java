...
public class CustomerUI extends JFrame {
	...
	public CustomerUI() {
		...
		JLabel lblNewLabel = new JLabel("客 戶 - 基本資料作業");
	...
	// end of
		...
		bntQuery.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				// System.out.println(strSelected);		// result: C001-台北中山北店
				String[] words = strSelected.split("-");
				String Customerno = words[0];
				// System.out.println(Customerno);
				
				Customer oneCustomer = new CustomerServiceImpl().findByCustomerno(Customerno);
				if (oneCustomer != null)
				{
					Tool.save(oneCustomer, "customerUI.txt");
					
					try {
						UpdateCustomerUI frame;
						frame = new UpdateCustomerUI();
						frame.setVisible(true);
						dispose();
